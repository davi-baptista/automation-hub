...
class Bot(DesktopBot):
    ...
    def action(self, execution=None):
        filepath = r'C:\Users\davi.inov\Desktop\Projetos\Departamento_Pessoal\03_dctfweb\data\EMPRESAS DCTFWEB.xlsx'
        ...
        pasta_saida_darfs = r'C:\Users\davi.inov\Desktop\Projetos\Departamento_Pessoal\03_dctfweb\output\DARFs'
    ...
    def visualizar(self, codigos, nome, pasta_downloads, pasta_saida, contador=0):
        ...
        if contador == 0:
            self.selecionar_todos(contador)
            
            for codigo in codigos:
                self.procurar_e_marcar_codigo(codigo)
                
            status_pessoal = self.emitir_darf_e_salvar(nome, pasta_downloads, pasta_saida, contador)
            return status_pessoal
        
        elif contador == 1:
            self.selecionar_todos(contador)
            status_fiscal = 'Nenhum credito encontrado'
            
            for codigo in codigos:
                pyautogui.hotkey('ctrl', 'shift', 'k')
                self.procurar_e_marcar_codigo(codigo)
                status = self.emitir_darf_e_salvar(nome, pasta_downloads, pasta_saida, contador, codigo)
                self.procurar_e_marcar_codigo(codigo)
                
                if status == 'Guia gerada com sucesso':
                    status_fiscal = status
            return status_fiscal
        
        return 'Erro encontrado'
            
            
    def selecionar_todos(self, contador):
        contador += 1
    ...
    def procurar_e_marcar_codigo(self, codigo):
        codigo_javascript = "{"
    ...
    def emitir_darf_e_salvar(self, nome, pasta_downloads, pasta_saida, contador, codigo=0):
        ...
        self.recent_download(nome, pasta_downloads, pasta_saida, contador, codigo)
    ...
    def recent_download(self, company_name, pasta_downloads, caminho_saida_darf, contador, codigo=0):
        ...
        if arquivos:
            ...
            if codigo == 0:
                ...
                caminho_destino = os.path.join(caminho_saida_darf, company_name, departamento, codigo, os.path.basename(arquivo_mais_recente))
                os.makedirs(os.path.join(caminho_saida_darf, company_name, departamento, codigo), exist_ok=True)
