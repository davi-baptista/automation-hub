...
class NiboDownloader(WebBot):
        ...
        for index, row in df.iterrows():
            ...
            for month in self.months_marked:
                try:
                    ...
                    status, downloads_number = self.download_archives(results_path)
                    files_number = self.move_files(results_path, company, self.year_marked, month)

                    print(downloads_number)
                    print(files_number)
    ...
    def download_archives(self, results_path):
        buttons = self.find_elements('bank-statement-files', By.CLASS_NAME)
        if buttons:
            for button in buttons:
                button.click()
        
        downloads = self.find_elements('fa-cloud-download', By.CLASS_NAME)    
        if downloads:
    ...
    def move_files(self, results_path, company, year, month):
        # Diretorio que vamos enviar nossos arquvios
